 ...
 * 判断字符串[s]是否与当前模式匹配
 */
fun Matcher.match(s: String) = parse(s, 0).any { i -> i == s.length }

/**
 * 连接两个Matcher
 */
infix fun Matcher.and(rhs: Matcher) = Matcher { s, index ->
 ...
 */
infix fun Matcher.and(s: String) = and(str(s))

/**
 * 将字符[c]转换成Matcher，并与当前Matcher相连
 */
infix fun Matcher.and(c: Char) = and(ch(c))

/**
 * 使用or连接两个Matcher
 */
infix fun Matcher.or(rhs: Matcher) = Matcher { s, index ->
 ...
 */
infix fun Matcher.or(s: String) = or(str(s))

/**
 * 将字符[c]转换成Matcher，并使用or与当前Matcher相连
 */
infix fun Matcher.or(c: Char) = or(ch(c))

/**
 * 将当前Matcher连续应用至少[minTimes]次
 */
fun Matcher.many(minTimes: Int = 0) = Matcher { s, index ->
    sequence {
        // 应用minTimes次
 ...
 * 将当前Matcher连续应用1次或多次
 */
fun Matcher.many1() = many(1)

/**
 * 将当前Matcher重复最少[minTimes]次，最多[maxTimes]次
 */
fun Matcher.repeat(minTimes: Int, maxTimes: Int) = Matcher { s, index ->
    sequence {
        // 应用minTimes次
 ...
 * 将当前解析器重复[times]次
 */
fun Matcher.repeat(times: Int) = repeat(times, times)

/**
 * 应用当前Matcher，并调用[mapper]生成下一个Matcher，继续应用下一个Mapper
 */
fun Matcher.flatMap(mapper: (String) -> Matcher) = Matcher { s, index ->
